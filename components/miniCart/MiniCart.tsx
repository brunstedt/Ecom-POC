    ...
    return (
        <>
            <button onClick={() => setIsOpen(true)} className="text-white flex items-center gap-3 text-xl tracking-wider border-white border rounded-full px-4 py-1 md:-mb-2 md:mt-0 -mt-1 hover:bg-white hover:bg-opacity-20">
                ...
                Bokning {cart.cart.items.length > 0 ? <div className="text-sm bg-pink-500 w-6 font-bold flex items-center justify-center h-6 rounded-full">{cart.cart.items.length}</div> : null}
            </button>

            <Drawer isOpen={isOpen} onClose={() => setIsOpen(false)} title="Min bokning" backDrop>
                <div className="flex grow flex-col overflow-y-auto">
                    {cart.cart.items.length > 0 ? 
                        <div className="">{cart.cart.items.map(item => <MiniCartItem key={item.id} {...item} />)}</div> : 
                        <div className="flex justify-center py-24 text-xl text-gray-700">Inga bokningar gjorda</div>
                    }
                </div>
                    ...
                    <div>
                        {cart.cart.totals.discountTotal > 0 ? 
                            <>
                                ...
                                <div className="text-red-600 text-sm font-semibold mb-2">Rabatt: -{localeCurrency({amount: cart.cart.totals.discountTotal})}</div>
                                <div className='font-semibold text-lg'>
                                    Totalt: {localeCurrency({amount: cart.cart.totals.grandTotal})}
                                </div>
                            </>
                            : null}
                    </div>
